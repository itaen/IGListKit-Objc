...
- (NSString *)urlString {
    if (!_urlString) {
		_urlString=[NSString stringWithFormat:@"https://unsplash.it/%i/%@/",(int)[self.collectionContext containerSize].width,_height];
    }
    return _urlString;
}
...
-(UICollectionViewCell *)cellForItemAtIndex:(NSInteger)index{
    ...
    if (index==0) {
        [(LabelCell *)cell label].text = [NSString stringWithFormat:@"%@ sec:%zi",self.urlString,self.section];
    }
    else{
...
///  通知代表控制器将进入工作范围
/// @param listAdapter <#listAdapter description#>
/// @param sectionController <#sectionController description#>
-(void)listAdapter:(IGListAdapter *)listAdapter sectionControllerWillEnterWorkingRange:(IGListSectionController *)sectionController{
    ...
    NSInteger section = [listAdapter sectionForSectionController:sectionController];
...
/**
 ...
 @param listAdapter 控制列表的适配器。
 @param sectionController 退出范围的段控制器。
*/
